...
class SppAdult() {
    ...
    companion object{
        ...
        get()= mutableListOf(
                TestDto(0,"Всё, что я делаю, я делаю ради моего сына (дочери)", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.gpl),
                TestDto(1,"У меня часто не хватает времени позаниматься с сыном (дочерью) – пообщаться, поиграть.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.gmin),
                TestDto(2,"Мне приходится разрешать моему ребёнку такие вещи, которые не разрешают многие другие родители.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.ypl),
                TestDto(3,"Не люблю, когда сын (дочь) приходит ко мне с вопросами. Лучше, чтобы догадался сам (сама).", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.ymin),
                TestDto(4,"Наш ребёнок имеет больше обязанностей – в уходе за собой, поддержании порядка, чем большинство детей его возраста.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.tpl),
                TestDto(5,"Моего ребёнка трудно заставить что-нибудь сделать, чего он не любит.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.tmin),
                TestDto(6,"Всегда лучше, если дети не думают о том, правильно ли поступают их родители.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.zpl),
                TestDto(7,"Мой сын (дочь) легко нарушает запреты.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.zmin),
                TestDto(8,"Если хочешь, чтобы твой(я) сын (дочь) стал(а) человеком, не оставляй безнаказанным ни одного его (ее) плохого поступка.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.cpl),
                ...
                TestDto(90,"Когда наш сын (дочь) что-то натворит, мы беремся за него (ее). Если всё тихо, мы опять оставляем его (ее) в покое.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.n),
                ...
                TestDto(119,"Большую часть своего времени сын (дочь) проводит вне дома – в яслях, детском саду, в школе, у родственников.", mutableListOf("+", "-", "не знаю"), mutableListOf(1,0,0), SppBallType.gmin),
